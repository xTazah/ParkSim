...
class RealtimeVisualizer(object):
    ...
    def __init__(
        ...
        self.map_window = dpg.add_window(
            width=self.map_width + 50, height=self.map_height + 50, label="Map"
        )
        ...
        # Control Window
        self.control_window = dpg.add_window(
            width=100,
            height=200,
            pos=[self.map_width + 50, 0],
            label="Simulation Control",
            autosize=True,
        )

        self.start_pause_btn = dpg.add_button(
            label="Start/Pause", callback=self.running_cb, parent=self.control_window
        )

        # Running Status
        self.running = True
        self.running_display = dpg.add_text(
            default_value="Runing: " + str(self.running), parent=self.control_window
        )
        self.fps = 0
        self.fps_display = dpg.add_text(
            default_value="FPS: " + str(self.fps), parent=self.control_window
        )
        self.fps_list = []
    ...
    def _draw_waypoints(self):
        ...
        for _, points in self.dlpvis.waypoints.items():
            ...
            for px, py in zip(pxs, pys):
                dpg.draw_circle(
                    [px, py], 2, fill=(0, 0, 0, 128), parent=self.scene_canvas
                )
    ...
    def _draw_obstacles(self, scene_token):
        ...
        for obstacle_token in scene["obstacles"]:
            ...
            corners = self.dlpvis._get_corners(
    ...
    def draw_circle(self, center: np.ndarray, radius: float, color: tuple):
        ...
        dpg.draw_circle(
            center=[px, py], radius=radius, fill=color, parent=self.frame_canvas
        )
    ...
    def draw_text(
        self, position, text: str, size: float = 15, color: tuple = (0, 0, 0, 255)
    ):
        """
        ...
        dpg.draw_text(
            pos=[px, py], text=text, color=color, size=size, parent=self.frame_canvas
        )
...
if __name__ == "__main__":
    ...
    vis = RealtimeVisualizer(ds, vehicle_body, use_obstacles=True)
