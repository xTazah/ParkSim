...
import numpy as np
from parksim.intent_predict.cnn.data_processing.create_dataset import (
    ENTRANCE_TO_PARKING_LOT,
)
...
class Predictor:
    ...
    def load_model(
        self,
        waypoints: WaypointsGraph,
        model_path=ROOT_DIR
        / "models/smallRegularizedCNN_L0.068_01-29-2022_19-50-35.pth",
    ):
        self.waypoints = waypoints
        model = SmallRegularizedCNN()
        if self.use_cuda:
            model_state = torch.load(model_path, map_location=torch.device("cuda"))
        else:
    ...
    def predict(
        self, instance_centric_view, global_position, heading, speed, time_spent_in_lot
    ) -> PredictionResponse:
        ...
        # start = time.time()
        image_features, non_spatial_features, spot_coordinates = self.get_features(
            instance_centric_view, global_position, heading, speed, time_spent_in_lot
        )
        # end = time.time()
        ...
        img_tensors = [
            transforms.ToTensor()(img_feature) for img_feature in image_features
        ]
        img_tensors = torch.stack(img_tensors, 0)
        non_spatial_tensors = torch.stack(
            [
                torch.Tensor(non_spatial_feature.astype(np.single))
                for non_spatial_feature in non_spatial_features
            ],
            0,
        )
        if self.use_cuda:
        ...
        if chosen_index == len(spot_coordinates):
            response = PredictionResponse(
                False, None, normalized_scores, spot_coordinates
            )
        else:
            response = PredictionResponse(
                True,
                spot_coordinates[chosen_index],
                normalized_scores,
                spot_coordinates,
            )
        return response
    ...
    def get_features(
        ...
        non_spatial_features.append(
            np.array([[0, 0, ego_speed, distance_to_entrance, time_spent_in_lot]])
        )
    ...
    def detect_spot_centers(
        self, instance_centric_view, global_position, heading, speed
    ):
        """
        ...
        current_state = np.array(
            [global_position[0], global_position[1], heading, speed]
        )
        for rect in boxes:
            center_pixel = np.array(rect[0])
            center_ground = self.local_pixel_to_global_ground(
                current_state, center_pixel
            )
    ...
    def compute_Astar_dist_dir(
        self, vehicle_coords, target_coords: np.ndarray, heading
    ):
        """
        ...
        if spot_vertex_idx == current_vertex_idx:
            ...
            path_vector = (
                astar_graph.vertices[1].coords - astar_graph.vertices[0].coords
            )
    ...
    def get_time_spent_in_lot(self, ds, agent_token, inst_token):
        ...
        instance_tokens_agent_is_in = [
            instance["instance_token"] for instance in instances_agent_is_in
        ]
